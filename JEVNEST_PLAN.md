...
## 2,000-question route screen

Added `experiments/jev/routes`: 200 authored intervention routes across 20 families, each with ten research questions (2,000 unique IDs). All are packed for real API collection, using an allowlisted historical production profile; the supplied four-job run produces 37 bounded requests. Unlike the earlier subset study, no route questions are silently filtered out. These are explicit speculative route judgments, not implemented interventions or measured speedups. Each route lists the evidence needed to test it. `questions:routes` prepares, existing `questions:collect` collects, and `questions:routes:report` writes the 200×10 response matrix and results.zip with complete/missing coverage. No live nesting policy changes.
